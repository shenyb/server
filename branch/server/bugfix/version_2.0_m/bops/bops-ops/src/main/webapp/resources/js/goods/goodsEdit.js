...
var topPicLength = $("#hiddenGoodsTopPicLength").val();
var detailPicLength = $("#hiddenGoodsDetailPicLength").val();
var picAddress = $("#hiddenPicAddress").val();
var currentPage = $("#hiddenPage").val();
var topPicArray = new Array();
var detailPicArray = new Array();
if('' != $("#hiddenTopPicKeys").val()){
	topPicArray = $("#hiddenTopPicKeys").val().split(",");
}
if('' != $("#hiddenDetailPicKeys").val()){
	detailPicArray = $("#hiddenDetailPicKeys").val().split(",");
}
...
$("#a_submitGoodsEditSave")
		.click(
				function() {
					/* alert($("#goodsEditProfile").serialize()); */
					// alert($("#goodsEditProfile").validate().form());
                    if($("#brief").val()==null||$("#brief").val().length==0){
                    	alert("商品简介不能为空");
                    	return false;
                    	
                    } 
                    else if($("#hiddenTopPicKeys").val()==null||$("#hiddenTopPicKeys").val().length==0){
                    	alert("商品主图也不能为空");
                    	return false;
                    	
                    } 
                    else if($("#hiddenScenePicKeys").val()==null||$("#hiddenScenePicKeys").val().length==0){
                    	alert("商品场景图还是不能为空的");
                    	return false;
                    	
                    } 
                    else if($("#hiddenDetailPicKeys").val()==null||$("#hiddenDetailPicKeys").val().length==0){
                    	alert("商品详情大图更不能为空");
                    	return false;
					...
					for (i = 0; i < topPicArray.length; i++) {
						hiddenTopPicKeys = hiddenTopPicKeys + ","
								+ topPicArray[i];
					}
					if (hiddenTopPicKeys.charAt(hiddenTopPicKeys.length - 1) == ',') {
						hiddenTopPicKeys = hiddenTopPicKeys.substring(0,
					...
					for (i = 0; i < detailPicArray.length; i++) {
						hiddenDetailPicKeys = hiddenDetailPicKeys + ","
								+ detailPicArray[i];
					}
					if (hiddenDetailPicKeys
							.charAt(hiddenDetailPicKeys.length - 1) == ',') {
						hiddenDetailPicKeys = hiddenDetailPicKeys.substring(0,
								hiddenDetailPicKeys.length - 1);
					}
					...
					$.ajax({
						type : "post",
						url : "/goods/goodsEditSave",
						data : $("#goodsEditProfile").serialize(),
						cache : false,
						dataType : 'json',
						success : function(data) {
							if (data.code == 200) {
								window.location.href = "/goods/page?page=" + currentPage;
							} else {
