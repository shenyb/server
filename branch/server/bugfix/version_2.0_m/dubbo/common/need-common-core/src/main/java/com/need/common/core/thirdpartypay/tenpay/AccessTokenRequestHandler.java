...
import com.need.common.core.thirdpartypay.tenpay.util.WXUtil;
...
public class AccessTokenRequestHandler extends RequestHandler {
	...
	protected static String getTokenReal() {
		String requestUrl = ConstantUtil.TOKENURL + "?grant_type=" + ConstantUtil.GRANT_TYPE + "&appid="
				+ PropertiesUtil.getProperty("/properties/wechat.properties", "app_id") + "&secret="
				+ PropertiesUtil.getProperty("/properties/wechat.properties", "app_secret");
		;
		String resContent = "";
	...
	private static boolean tokenIsExpire(String access_token) {
		...
		wxReqHandler.setParameter("appid", PropertiesUtil.getProperty("/properties/wechat.properties", "app_id"));
		wxReqHandler.setParameter("appkey", PropertiesUtil.getProperty("/properties/wechat.properties", "private_key"));
