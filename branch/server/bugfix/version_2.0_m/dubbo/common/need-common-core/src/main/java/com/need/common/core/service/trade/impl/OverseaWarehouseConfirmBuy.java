 ...
 * </p>
 * 
 * @author shenyb 2015年12月3日 下午2:30:58
 * @ClassName OverseaWarehouseConfirmBuy
 * @Description 海外仓单独处理 * 运费：按照商品总价计算 佣金：按照支付总价计算 优惠券：按照使用佣金后的总支付计算
 * @version V2.0
 ...
 */
@Component
public class OverseaWarehouseConfirmBuy extends ConfirmBuyAbstract implements InitializingBean {
	private static final Logger LOGGER = LoggerFactory.getLogger(OverseaWarehouseConfirmBuy.class);
	...
	protected Message generateTrade(CreateTradeBaseParamVO param) {
		...
		record.setWarehouseType(WarehouseTypeEnum.OVERSEA_WAREHOUSE.code);
		...
		Message transportFeeMessage = getTransportFee(resultList, userId, WarehouseTypeEnum.OVERSEA_WAREHOUSE,
				totalSum);
	...
	/**
	 * @author shenyb 2015年12月4日 下午5:49:40 @Method:
	 *         checkGoodsStore @Description: TODO @param records @throws
	 */
	...
	protected Message useCommission(List<TradeBasePO> trades, String userId, int totalPrice, int useCommissionLimit) {
		Message message = Message.success();
		int commission = userCommissionAccountService.getAvailableBalance(userId, useCommissionLimit);
		if (commission > 0) {
			for (int i = 0; i < trades.size(); i++) {
				if (i == 0) {
					trades.get(i).setCommission(commission);
				}
				trades.get(i).setPayPrice(trades.get(i).getPayPrice() - commission);
			}
		}
		return message;
	}
