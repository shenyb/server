				               ...
				               <c:forEach items="${list}" var="list" varStatus="status">
									<tr class="odd" role="row">
										...
										<td>
											...
											<c:if test='${list.purchaseStatus == "HAVEST_HALF"}'>部分收货</c:if>
											<c:if test='${list.purchaseStatus == "HAVEST_ALREADY"}'>收货完成</c:if>
										</td>
										...
										<td><c:if test='${list.differenceCount == "0" && list.purchaseStatus != "WAIT_HARVEST"}'>否</c:if>
											<c:if test='${list.differenceCount != "0" && list.purchaseStatus != "WAIT_HARVEST"}'>是</c:if></td>
										<td>
											<c:if test='${list.dredge == "2" && list.purchaseStatus == "WAIT_HARVEST"}'><a href="/purchase/toModPurchase?purchaseId=${list.purchaseId}">收货</a></c:if>
										</td>
									</tr>
								</c:forEach>
				                </tbody>
