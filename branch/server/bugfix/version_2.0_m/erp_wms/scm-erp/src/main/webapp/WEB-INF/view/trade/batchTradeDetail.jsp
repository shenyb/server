...
<link rel="stylesheet" href="/resources/css/datapa ge.css">
<!-- 分页插件 css 样式 -->
...
<body>
					        ...
					        <div class="info">
					            <div class="info-top">
					                ...
					                <div class="back">
					                    ...
					                    <a href="javascript:;" id="exportBatch" onClick="daochuBatch()" class="button button-primary button-raised button-small">导出</a>
					                </div>
					            </div>
					            ...
					            <table class="table table-bordered table-hover">					
					                ...
					                <c:forEach items="${list}" var="trade" varStatus="status">
										<tr>
											<td style="text-align:left;">${trade.batchNo}</td>
											<td style="text-align:left;">${trade.tradeNo}</td>
											<td>${trade.logisticsNo}</td>
											<td>
												<c:if test="${trade.payChannel=='WECHAT' }">微信</c:if>
												<c:if test="${trade.payChannel=='ALIPAY' }">支付宝</c:if>
											</td>
											<td>${trade.pushReturnStatus}</td>
											<td>
												<c:if test="${trade.payChannel=='WECHAT' }">${trade.wechatRetrieveStatus}</c:if>
												<c:if test="${trade.payChannel=='ALIPAY' }">${trade.alipayRetrieveStatus}</c:if>
											</td>
							...
							<div class="wrapper">
								<div class="row">
									<div class="col-sm-12">
										<header class="panel-heading ">
											批次详情列表 <span class="tools pull-right"> <a
												class="fa fa-repeat box-refresh"
												href="javascript:javascript:window.location.reload();"></a>
											</span>
										</header>
										<div
                                	...
                                	支付宝
															...
															<td>${trade.pushReturnStatus}</td>
															<td>${trade.alipayRetrieveStatus}</td>
															<td>${trade.retrieveStatus}</td>
	...
	<script type="text/javascript">
		...
		function daochuBatch(){
			var url = "/tradeBatch/exportBatch?";
			url = url + "batchNo=" + $("#batchNo").val();
			autoDowload(url);
		}
