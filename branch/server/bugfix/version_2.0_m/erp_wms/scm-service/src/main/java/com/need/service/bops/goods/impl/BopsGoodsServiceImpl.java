...
//	private BopsGoodsStoreDAO bopsGoodsStoreDAO;
	...
	public int insertStore(String goodsId){
		BopsGoods goods = bopsGoodsDAO.getByGoodsId(goodsId);
		BopsInventoryPO bopsInventory = new BopsInventoryPO();
		bopsInventory.setGoodsCode(goods.getGoodsCode());
		bopsInventory.setGoodsId(goodsId);
		bopsInventory.setWarehouseId(Integer.parseInt(WarehouseTypeEnum.getWarehouseId(goods.getWarehouseType())));
		bopsInventory.setCreatedBy(String.valueOf(goods.getPublisherId()));
		bopsInventory.setChangeBy(String.valueOf(goods.getPublisherId()));
		bopsInventory.setPurchasePrice(goods.getPurchasePrice());
		return bopsInventoryDAO.addNewStore(bopsInventory);
	}
	...
	public void auditGoods(AuditGoodsVO bopsGoods) {
		...
		BopsGoods goods = bopsGoodsDAO.selectByPrimaryKey(bopsGoods.getGoodsId());
		// 跟新后台库
	...
	public void addOrEditPortalGoods(String bopsGoodsId) {
		...
		if ("MORE".equals(bopsGoods.getGoodsType())) {
			...
			if (goodsPortal != null) {
				...
				goodsMainDAO.updateByPrimaryKey(goodsMain);
				if (null != bopsGoodsDetail) {
	...
	public GoodsStoreDetailVO viewGoods(String goodsId) {
		...
		GoodsStoreDetailVO goodsStoreDetailVO = new GoodsStoreDetailVO();
		//根据最后是上架人id,查询姓名
		if(null != bopsGoods.getUpdateOnlineId()){
			if(0 == bopsGoods.getUpdateOnlineId().intValue()){
				goodsStoreDetailVO.setUpdateOnlineName(Constants.SUPERADMIN);
			}else{
				BopsUser BopsUser=bopsUserDAO.selectByPrimaryKey(bopsGoods.getUpdateOnlineId());
				goodsStoreDetailVO.setUpdateOnlineName(BopsUser.getUserRealName());
			}
		}
	...
	public HSSFWorkbook getGoodsbook(List<GoodsStoreDetailResultVO> data) {
		...
		for (int i = 1; i < data.size() + 1; i++) {
			...
			//根据商品id查询销量
			BopsGoodsSales bopsGoodsSales = bopsGoodsSalesDAO.selectByPrimaryKey(po.getGoodsId());
			if(null != bopsGoodsSales){
				cell = ExcelUtil.createCell(row, 16, cellStylec);
				if(null == bopsGoodsSales.getSalesServen()){
					cell.setCellValue("0");
				}else{
					cell.setCellValue(bopsGoodsSales.getSalesServen().intValue());
				}
				cell = ExcelUtil.createCell(row, 17, cellStylec);
				if(null == bopsGoodsSales.getSalesFifteen()){
					cell.setCellValue("0");
				}else{
					cell.setCellValue(bopsGoodsSales.getSalesFifteen().intValue());
				}
				cell = ExcelUtil.createCell(row, 18, cellStylec);
				if(null == bopsGoodsSales.getSalesThirty()){
					cell.setCellValue("0");
				}else{
					cell.setCellValue(bopsGoodsSales.getSalesThirty().intValue());
				}
				cell = ExcelUtil.createCell(row, 19, cellStylec);
				if(null == bopsGoodsSales.getSalesSixty()){
					cell.setCellValue("0");
				}else{
					cell.setCellValue(bopsGoodsSales.getSalesSixty().intValue());
				}
				cell = ExcelUtil.createCell(row, 20, cellStylec);
				if(null == bopsGoodsSales.getSalesNinety()){
					cell.setCellValue("0");
				}else{
					cell.setCellValue(bopsGoodsSales.getSalesNinety().intValue());
				}
			}
	...
	public BopsGoodsItemsPO insertGoodsItems(List<BopsGoodsItemsVO> list, BopsGoods bopsGoods, GoodsParamsVO paramsVO) {
		...
		bopsGoodsDetail.setGoodsDesc("");
		bopsGoodsDetail.setDetailPicKeys("");
		bopsGoodsDetail.setGoodsParams(goodsParamsJSONString);
		bopsGoodsDetailDAO.insertNewGoods(bopsGoodsDetail);// 插入商品扩展表
	...
	public void updateGoodsStatus(BopsGoods bopsGoods) {
		bopsGoodsDAO.updateOnlineGoods(bopsGoods);
