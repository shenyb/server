...
import com.need.schedule.ConsoleManager;
import com.need.schedule.local.DynamicTaskManager;


/**
 * @author  Git@OSC 2015年12月17日 下午8:38:53
 * @ClassName ScheduleDataManager4ZK
 * @Description zk实现类
 * @version V1.0   
...
public class ScheduleDataManager4ZK implements IScheduleDataManager {
	...
	public void clearExpireScheduleServer() throws Exception{
		...
		for (String name : this.zkManager.getZooKeeper().getChildren(zkPath, false)) {
			try {
				Stat stat = new Stat();//节点的状态信息
				this.getZooKeeper().getData(zkPath + "/" + name, null, stat);
				if (getSystemTime() - stat.getMtime() > SERVER_EXPIRE_TIME) {//需要优化
	...
	private void assignServer2Task(List<String> taskServerList, String taskPath)throws Exception {
		int index = random.nextInt(taskServerList.size());//更换成 一致性hash实现
	...
	public static String getLeader(List<String> serverList){
	...
	public static void main(String[] args) throws Exception {
		...
		serverList.add("10.170.184.234$0383E6863C3A41A6A4E08B0661305BE6$0000000054");
		serverList.add("127.0.0.1$128EF4C808124D2D95D43CCA4AB9CB6F$0000000055");
		serverList.add("127.0.0.1$98FF84151788436CA90ED05D19F3C39B$0000000056");
		
		
		System.out.println(ScheduleDataManager4ZK.getLeader(serverList));
