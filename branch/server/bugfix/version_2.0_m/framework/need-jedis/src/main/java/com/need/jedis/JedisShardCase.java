 ...
 */
public class JedisShardCase extends AbstractConfigHandler<Object>{
	/**
	 * shard client
	 */
	...
	protected void initialShardedPool() {
		...
		for(Node node : nodes){
		    System.out.println("init jedis pool----------:"+node.getIp()+":"+node.getPort()+"&"+node.getPassword());
		    //modify by liyongran 20151201 添加密码
		    JedisShardInfo jedisShardInfo= new JedisShardInfo(node.getIp(), Integer.parseInt(node.getPort()), "master_"+node.getIp());
		    jedisShardInfo.setPassword(node.getPassword());
...
//		}
	...
	/**
	 * get the case from pool
	 * @return
	 */
	public ShardedJedis getShardJedisCase(){
	    shardedJedis = shardedJedisPool.getResource();
	    System.out.println("shardedJedis:"+shardedJedis);
	...
	protected void returnPool(){
        shardedJedisPool.returnResource(this.shardedJedis);
    }
    ...
    public boolean batchIncrease(String [] keys)
    {
        boolean result = true;
        try {
        	ShardedJedisPipeline p = getShardJedisCase().pipelined();
...
//            p.sync();
        	 ...
        	 result = false;
             shardedJedisPool.returnBrokenResource(getShardJedisCase());  
         }finally{
             returnPool();
         }
        return result;
