...
package com.need.marketing.service.weixin.impl;
...
import com.need.marketing.constant.RedisKeyConstants;
...
public class WeixinServiceImpl implements WeixinService {
    ...
    private final String appid = PropertiesUtil.getProperty("/properties/wechat.properties", "app_id");
    private final String secret = PropertiesUtil.getProperty("/properties/wechat.properties", "app_secret");
	...
	public String getAccessToken() {
		 ...
		 */
		String access_token = JedisSentinelClient.getString(RedisKeyConstants.WEIXIN_MP_ACCESS_TOKEN);
		if (StringUtils.isBlank(access_token)) {
			...
			try {
				...
				if (StringUtils.isNotBlank(access_token)) {
                    JedisSentinelClient.setString(RedisKeyConstants.WEIXIN_MP_ACCESS_TOKEN, access_token, RedisKeyConstants.WEIXIN_MP_ACCESS_TOKEN_EXPIRE);// 放在缓存里，失效时间30分钟;
				}
				return access_token;
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
    ...
    public  String getJsTicket(String token){
    	 ...
    	 * 从redis获取
    	 */
    	String jsapiTicket = JedisSentinelClient.getString(Constants.JSAPI_TICKET);
    	...
    	try {
           String jsticketUrl = "https://api.weixin.qq.com/cgi-bin/ticket/getticket";
			...
			if (StringUtils.isNotBlank(jsapiTicket)) {
				JedisSentinelClient.setString(Constants.JSAPI_TICKET, jsapiTicket, Constants.TOKEN_TIME_OUT_MIN);// 放在缓存里，失效时间90分钟;
			}
