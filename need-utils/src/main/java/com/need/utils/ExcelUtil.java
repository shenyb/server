...
public class ExcelUtil {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 10; i++) {
			HSSFRow row = ExcelUtil.createRow(sheet,i,500);
			for (int j = 0; j < 3; j++) {
				Font font = ExcelUtil.createFont(workbook, (short)10, HSSFColor.BLACK.index, (short)20);
				CellStyle cellStylec = ExcelUtil.createBorderCellStyle(workbook, HSSFColor.WHITE.index, HSSFColor.BLUE.index, (short)30, font);
				HSSFCell cell = ExcelUtil.createCell(row, j, cellStylec);
				cell.setCellValue("haha-"+RandomUtils.nextInt(100));
			}
		}
		ExcelUtil.writeWorkbook(workbook, "test.xls");
