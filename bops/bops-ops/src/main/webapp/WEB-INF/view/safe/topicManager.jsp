				        ...
				        <div class="info">
			                ...
			                <tbody>
							      <c:forEach items="${list}" var="comment" varStatus="status">
									<tr>
										<td class="sorting_1">${comment.commentId}</td>
										<td>${comment.feedId}</td>
										<td><fmt:formatDate value="${comment.createTime}"
												type="both" /></td>
										<td>${comment.comment}</td>
										<td><c:choose>
											<c:when test="${comment.feedStatus == 'VALID'}">未删除</c:when>
											<c:otherwise>已删除</c:otherwise>
											</c:choose>
									    </td>
										<td><a href="javascript:;" data-toggle="modal" data-target="#myModal" onclick="pushVal('${comment.commentId}')">删除</a></td>
									</tr>
								</c:forEach>
				                </tbody>
				            </table>
										...
										<thead>
											<tr role="row">
												<th
													...
													rowspan="1" aria-controls="DataTables_Table_0" tabindex="0"
													class="sorting_asc">commentID</th>
...
<script type="text/javascript">
	...
	function del(){
		...
		to_ajax('/feedComment/'+fid_val,'DELETE',{},callback,'服务器忙，请稍后重试！');
	}
