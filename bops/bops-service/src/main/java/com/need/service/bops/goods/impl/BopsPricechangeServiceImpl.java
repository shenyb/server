...
public class BopsPricechangeServiceImpl implements BopsPricechangeService {
	...
	public List<GoodsPriceResultVO> queryPriceList(int pricechangeId) {
		...
		if(priceList != null && priceList.size() > 0){
			for(GoodsPriceResultVO resultVO: priceList){
				resultVO.setProfit(((resultVO.getDiscountPrice() == null ? 0D : resultVO.getDiscountPrice()) 
						- (resultVO.getPurchasePrice() == null ? 0D : resultVO.getPurchasePrice()))/100);
				resultVO.setDiscountPrice(resultVO.getDiscountPrice() == null ? 0D : resultVO.getDiscountPrice()/100);
				resultVO.setOriginalPrice(resultVO.getOriginalPrice() == null ? 0D : resultVO.getOriginalPrice()/100);
				resultVO.setPurchasePrice(resultVO.getPurchasePrice() == null ? 0D : resultVO.getPurchasePrice()/100);
			}
		}
	...
	public int updateExecute(String excuted, Integer pricechangeId){
		if("ENDED".equals(excuted)){
			changPrice(pricechangeId, "end");
			bopsPricechangeDAO.updateEndTime(pricechangeId);
		}
		else if("STARTED".equals(excuted)){
			changPrice(pricechangeId, "start");
	...
	public int updateAudit(String audit, Integer pricechangeId, Integer auditorId){
		...
		if("VALID".equals(audit) && "EXCUTE".equals(pricechange.getPricechangeType())){
			changPrice(pricechangeId, "start");
			bopsPricechangeDAO.updateExcuted("STARTED", pricechangeId);
		}
		return result;
	}
