...
public class BopsPurchaseServiceImpl implements BopsPurchaseService {
	...
	public BopsPurchaseVO getPurchaseDetail(String id) {
		...
		if(null != bopsPurchasePO.getDifferencePrice()){
			bopsPurchaseVO.setDifferencePrice(Double.parseDouble(CurrencyUtil.fromFenToYuan(bopsPurchasePO.getDifferencePrice().toString())));
		}
	 ...
	 * @throws
	 */
	public void getMessageToSave(ESynEdiReceiveMessage eSynEdiReceiveMessage){
		...
		if(null == bopsReceiveSuccess){
			try {
				...
				for (PurchaseItemInVO purchaseItemInVO : itemList) {
					...
					storeChangeVO.setNowStoreCount(bopsGoods.getHavestInfoNormal());
					storeChangeVO.setStoreType("PURCHASE");
					...
					bopsGoodsStoreService.saveStoreChange(storeChangeVO);
					//实际总数量
					purchaseCountReal=purchaseCountReal+bopsGoods.getHavestInfoDisability()+bopsGoods.getHavestInfoNormal();
					//根据商品code和采购单号查询采购单价
					BopsPurchaseInfoPO priceInfo=bopsPurchaseInfoDAO.selectPrice(bopsGoods);
					//实际总金额
					purchasePriceReal=purchasePriceReal+priceInfo.getPurchasePrice().intValue()*purchaseCountReal;
				}
