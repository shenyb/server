(function($){
	var bigAutocomplete = new function(){
		...
		this.autocomplete = function(param){
			...
			this.config = {
			               ...
			               url:null,
			               /*data：格式{data:[{brandName:null,result:{}},{brandName:null,result:{}}]}
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						for(var i=0;i<data.length;i++){
							if(data[i].brandName.indexOf(keyword_) > -1){
								data_.push(data[i]);
							}
						}
			...
			function makeContAndShow(data_){
				...
				for(var i=0;i<data_.length;i++){
					cont += "<tr><td><div>" + data_[i].brandName + "</div></td></tr>"
				}
				cont += "</tbody></table>";
	...
	$(document).on('mousewheel', function(ev) {	bigAutocomplete.hideAutocomplete();});
	$(document).on('DOMMouseScroll', function(ev) {	bigAutocomplete.hideAutocomplete();});
