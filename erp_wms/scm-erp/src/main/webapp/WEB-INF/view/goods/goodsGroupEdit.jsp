...
<body>
									...
									<form id="goodsEditProfile" class="form-horizontal">
											<div class="info">
									            ...
									            <header class="panel-heading">
													...
													<div class="back">
									                    ...
									                    <tr>
									                    <input type="hidden" name="hiddenGoodsCode" id="hiddenGoodsCode" value="${bopsGoods.goodsCode}">
									                        <td><i class="fa fa-asterisk"></i>商品名称</td>
									                        <td><input type="text" name="goodsName" id="goodsName" value="${bopsGoods.goodsName}" class="form-control" required></td>
									                        <td><i class="fa fa-asterisk"></i>商品仓库</td>
									                        <td>
									                        	<select  class="form-control" required name="warehouseType" id="warehouseType">
																	<option value="SELF_WAREHOUSE" <c:if test="${bopsGoods.goodsStatus == 'SELF_WAREHOUSE'}">checked</c:if>>自营仓</option>
																	<option value="OVERSEA_WAREHOUSE" <c:if test="${bopsGoods.goodsStatus == 'OVERSEA_WAREHOUSE'}">checked</c:if>>海外直邮</option>
																</select>
									                        </td>
									                    </tr>
									                    <tr>	
									                    	<td><i class="fa fa-asterisk"></i>商品短名</td>
									                        <td><input type="text" name="shortName" id="shortName" value="${bopsGoods.shortName}" class="form-control" required></td>                     
									                    </tr>
														<tr>
									       ...
									       <table id="goods" class="table table-bordered table-hover table-condensed table-responsive">    									         
									                 ...
									                 <c:forEach  items="${itemList}" var="item" varStatus="status">
									                	<tr>
										                	<td><input type="text" name="goodsCodeItem" value="${item.goodsCode}" onBlur='getGoodsDetail(this)'/></td>
										                    <td>${item.goodsName}</td>
										                	<td><input type="text" name="goodsCount" value="${item.goodsCount}" onChange='changePrice()'/></td>
										                	<td>${item.goodsStatus}</td>
										                	<td>${item.storeCount}</td>
										                	<td>${item.discountPrice}</td>
										                	<td><a onClick=removeThis(this)>移除</a></td>
										                	<input type='hidden' name='onsalePriceItem' value='${item.onsalePrice}'>
										                	<input type='hidden' name='discountPriceItem' value='${item.discountPrice}'>
										                	<input type='hidden' name='purchasePriceItem' value='${item.purchasePrice}'>
										                	<input type='hidden' name='weightItem' value='${item.weight}'>
									                	</tr>
									                </c:forEach>
									       </table>
    ...
    function addGoods(){
    	...
    	var tr="<tr><td><input type='text' name='goodsCodeItem' value='' onBlur='getGoodsDetail(this)' onkeydown='selectDetail(this)'/></td> <td></td> <td><input type='text' name='goodsCount' value='1' onChange='changePrice()' onkeydown='selectPrice(this)'/></td> <td></td> <td></td> <td></td> <td><a onClick=removeThis(this)>移除</a></td><input type='hidden' name='onsalePriceItem' value=''><input type='hidden' name='discountPriceItem' value=''><input type='hidden' name='purchasePriceItem' value=''><input type='hidden' name='weightItem' value=''></tr>";
    	$("#goods").append(tr);
	}
    ...
    function getGoodsDetail(obj){
    	var preItem = $("input[name='goodsCodeItem']");
    	if(preItem.length > 1){
    		for(var i=0;i<preItem.length;i++){
    			var couponPre=$(preItem[i]).parent().parent().prev();
    	    	var pre=$(couponPre).css("background-color");
    	    	if(null != pre){
    	    		if(pre == "rgb(255, 51, 85)"){
    	        		alert("请修改或移除错误商品记录");
    	        		return false;
    	        	}
    	    	}
    ...
    function changePrice(){
		...
		for(var i=0;i<goodsCodeItem.length;i++){
			if("" == goodsCount[i].value || goodsCount[i].value == null ){
				goodsCount[i].value=1;
			}
    ...
    function doIt() {
		...
		var hiddenGoodsCode=$("#hiddenGoodsCode").val();
        $.ajax({
            type : "POST",
            url : "/goods/ModGoodsGroup",
            datatype : "json",
            data : {
            	dataAll : arr.toString(),
            	goodsName:goodsName,
