...
public class BopsGoodsServiceStoreImpl implements BopsGoodsStoreService {
	...
	public int addBopsStore(GoodsStorePageVO store){
		String goodsId = store.getGoodsId();
		BopsInventoryPO bopsInventory = bopsInventoryDAO.getByGoodsId(goodsId);
		BopsGoods goods = bopsGoodsDAO.getByGoodsId(goodsId);
		/**
		 * 未税成本 = 加权平均 = （入库数量*入库价格 + 在库数量*在库成本） / （入库数量+在库数量）
		 */
		int inStoreCount = bopsInventory.getNormalQuantity() + bopsInventory.getDemageQuantity();//在库数量
		bopsInventory.setCost((int)Math.ceil((double)(store.getNowStoreCount() * goods.getPurchasePrice() 
				+ (inStoreCount * bopsInventory.getCost())) * 1.0 / (store.getNowStoreCount() + inStoreCount) * 1.0));
		bopsInventory.setPurchasePrice(goods.getPurchasePrice());
		if("GOOD".equals(store.getStoreType())){
			bopsInventory.setNormalQuantity(store.getNowStoreCount());
			bopsInventoryDAO.updateNormalStore(bopsInventory);
		}
		else{
			bopsInventory.setDemageQuantity(store.getNowStoreCount());
			bopsInventoryDAO.updateDemageStore(bopsInventory);
		}
	...
	public List<GoodsStoreResultVO> pageBopsGoodsStore(GoodsStorePageVO page) {
		...
		for (GoodsStoreResultVO store : storeList) {
			...
			if(goodsMain != null){
				...
				store.setLockedCount(0);
				store.setOnWayCount(0);
				store.setNowStoreCount(0);
				bopsGoods.setGoodsType(bopsGoods.getGoodsType());
			}
...
//					}
	...
	public Message saveStoreChange(StoreChangeVO storeChangeVO) {
		...
		if(storeChangeVO.getDefectiveStore() == null){
			storeChangeVO.setDefectiveStore(0);
		}
		if(storeChangeVO.getNowStoreCount() == null){
			storeChangeVO.setNowStoreCount(0);
		}
		GoodsMainPO goodsMain = goodsMainDAO.selectByPrimaryKey(goodsId);
		if(null == goodsMain){
		...
		else if(StoreTypeEnum.REJECTION.code.equals(storeChangeVO.getStoreType())){
			addPortalGoodsCount(goodsId, storeChangeVO.getNowStoreCount());
		}
		else if(StoreTypeEnum.STOCKTAKING.code.equals(storeChangeVO.getStoreType())){
			addPortalGoodsCount(goodsId, storeChangeVO.getNowStoreCount());
		}
		storeChangeVO.setHistoryStore(goodsMain.getStoreCount());
		bopsGoodsStoreDAO.saveStoreChange(storeChangeVO);
		bopsInventoryDAO.addNormalStore(storeChangeVO);
		return message;
	}
