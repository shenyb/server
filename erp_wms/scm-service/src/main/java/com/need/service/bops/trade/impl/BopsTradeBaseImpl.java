...
import com.need.dao.bops.store.BopsInventoryFreezeLogDAO;
...
//	TradeLogisticsInfoDAO  tradeLogisticsInfoDAO;
	...
	@Autowired
	BopsInventoryDAO bopsInventoryDAO;
	@Autowired
	BopsInventoryFreezeLogDAO bopsInventoryFreezeLogDAO;
	@Override
	...
	public Message send(BopsTradeLogistics po) {
		 ...
		 * 扣减库存&记录流水
		 */
		Message resultMessage = reduceStore(tradeNo);
		if(resultMessage.getCode() != Message.SUCCESS){
	...
	public Message reduceStore(String tradeNo){
		...
		return resultCount > 0 ? message : Message.error(3063);
	}
	@Override
	@Transactional("bops_txManager")
	public Message reduceStoreOfFefund(List<TradeBasePO> tradeList){
		...
		if(tradeList != null && tradeList.size() > 0){
			for(TradeBasePO trade: tradeList){
				String  goodsId = trade.getGoodsId();
				bopsInventoryDAO.updateFreezeStore(goodsId, trade.getBuyCount());
				long bopsInventoryId = bopsInventoryDAO.selectByGoodsId(goodsId).getId();
				bopsInventoryFreezeLogDAO.saveStoreLog(bopsInventoryId, trade.getBuyCount()*(-1), trade.getOrderNo(), "待发货时客户退款扣减冻结库存流水记录");
			}
		}
	...
	public Message refund(String tradeNo, int refundAmount, String memo, BopsUser user) {
		...
		if (tradeList != null && tradeList.size() > 0) {
			...
			tradeRetrieveStatusRecord.setOrderNo(tradeBasePO.getUserOrderNo());
			tradeRetrieveStatusRecord.setTradeNo(tradeBasePO.getUserTradeNo());
			tradeRetrieveStatusRecord.setTrackingCode(TrackingCodeEnum.REFUND.code);
			tradeRetrieveStatusRecord.setTrackingDesc(TrackingCodeEnum.REFUND.desc);
			tradeRetrieveStatusRecord.setUpdateTime(new Date());
			tradeRetrieveStatusRecordDAO.insertRefund(tradeRetrieveStatusRecord);
		}
	...
	public HSSFWorkbook getTradeExportWorkBook(List<TradeExportVO> data){
		...
		cellStylecNum.setDataFormat(format.getBuiltinFormat("0.00"));
		HSSFRow row = ExcelUtil.createRow(sheet, 0, 500);
...
//		List<BopsGoodsCategoriesPO> categoryList = bopsGoodsCategoriesDAO.queryAll();
		...
		for (int i = 1; i < data.size() + 1; i++) {
			...
			cell = ExcelUtil.createCell(row, 2, cellStylecNum);
			cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getTotalPrice().intValue()+""));
			cell = ExcelUtil.createCell(row, 3, cellStylecNum);
			cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getPayPrice().intValue()+""));
			cell = ExcelUtil.createCell(row, 4, cellStylecNum);
			cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getGoodsPrice().intValue()+""));
			cell = ExcelUtil.createCell(row,5, cellStylecNum);
			cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getDiscountAmount().intValue()+""));
			cell = ExcelUtil.createCell(row, 6, cellStylecNum);
			cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getTransportFee().intValue()+""));
			cell = ExcelUtil.createCell(row, 7, cellStylec);
...
//			cell.setCellValue(categoryThree.getCategoryName());
	...
	public HSSFWorkbook getOrderItemBook(List<TradeExportItemVO> orderItemList) {
		...
		for (int i = 1; i < orderItemList.size() + 1; i++) {
			...
			cell = ExcelUtil.createCell(row, 7, cellStylecNum);
			cell.setCellValue(po.getPurchasePrice().doubleValue());
			cell = ExcelUtil.createCell(row, 8, cellStylecNum);
			...
			cell = ExcelUtil.createCell(row, 18, cellStylecNum);
			cell.setCellValue(po.getDiscount()==null?0:po.getDiscount().doubleValue()); //优惠
			cell = ExcelUtil.createCell(row, 19, cellStylecNum);
			cell.setCellValue(po.getFreight()==null?0:po.getFreight().doubleValue()); //运费
			cell = ExcelUtil.createCell(row, 20, cellStylecNum);
...
//			cell.setCellValue(categoryThree.getCategoryName());
	...
	@Override
	public HSSFWorkbook exportTradeList(TradeBaseParam param) {
		//param.setTotal(portalTradeBaseDAO.queryCountTradeExportByPage(param));
		List<TradeBaseVO> list = portalTradeBaseDAO.queryExportTrade(param);
		HSSFWorkbook workbook = getExportTradeList(list);
		return workbook;
	}
	public HSSFWorkbook getExportTradeList(List<TradeBaseVO> list){
			HSSFWorkbook workbook = new HSSFWorkbook();
			 SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			HSSFSheet sheet = ExcelUtil.createSheet(workbook, "订单列表数据");
			// 创建表头
			Font font = ExcelUtil.createFont(workbook, (short) 5, HSSFColor.BLACK.index, (short) 10);
			CellStyle cellStylec = ExcelUtil.createBorderCellStyle(workbook, HSSFColor.WHITE.index, HSSFColor.WHITE.index,
					(short) 30, font);
			...
			row.getSheet().setColumnWidth(1, 8000);
			row.getSheet().setColumnWidth(9, 8000);
			row.getSheet().setColumnWidth(6, 8000);
			HSSFCell cell = ExcelUtil.createCell(row, 0, cellStylec);
			cell.setCellValue("订单号");
			cell = ExcelUtil.createCell(row, 1, cellStylec);
			cell.setCellValue("商品总价");
			...
			for (int i = 1; i < list.size() + 1; i++) {
				...
				if(records!=null && records.size()>0){
					for(TradeRetrieveStatusRecord record :records){
						if(record.getTradeNo().equals(po.getUserTradeNo())){
							po.setTrackingStatus(record.getTrackingCode());
							break;
						}
					}
				}
				...
				cell.setCellValue(po.getUserTradeNo());
				cell = ExcelUtil.createCell(row, 1, cellStylec);
				cell.setCellValue(CurrencyUtil.fromFenToYuan(po.getTotalFee().intValue()+""));
				cell = ExcelUtil.createCell(row, 2, cellStylec);
				cell.setCellValue(po.getNickName()==null?"":po.getNickName());
				cell = ExcelUtil.createCell(row, 3, cellStylec);
				cell.setCellValue(po.getMobile());
				cell = ExcelUtil.createCell(row, 4, cellStylec);
				cell.setCellValue(TradeStatus.getDesc((po.getTradeStatus())));
				cell = ExcelUtil.createCell(row, 5, cellStylec);
				cell.setCellValue(PayChannelEnum.getMessage(po.getPayChannel()));
				cell = ExcelUtil.createCell(row, 6, cellStylec);
				cell.setCellValue(sdf.format(po.getTradeTime()));
				cell = ExcelUtil.createCell(row, 7, cellStylec);
				cell.setCellValue(WarehouseTypeEnum.getDesc(po.getWarehouseType()));
				cell = ExcelUtil.createCell(row, 8, cellStylec);
				cell.setCellValue(po.getTrackingStatus()==null?"":TrackingCodeEnum.getDesc(po.getTrackingStatus()));
				cell = ExcelUtil.createCell(row, 9, cellStylec);
				cell.setCellValue(po.getLogisticsValue());
			}
			return workbook;
}
