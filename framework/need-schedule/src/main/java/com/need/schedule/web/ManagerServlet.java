	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(StringUtils.isNotEmpty(del)){
			...
			if(StringUtils.isNotEmpty(period)){
				taskDefine.setPeriod(Long.valueOf(period)*1000);
			}
		...
		try {
			...
			if(servers != null){
	    		...
	    		for(int i=0; i< servers.size();i++){
	    			...
	    			String ser = servers.get(i);//modify by liyongran
	    			sb.append("<tr>")
	    		...
	    		out.println(String.format(FORM, request.getSession().getServletContext().getContextPath()+"/need/schedule"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
